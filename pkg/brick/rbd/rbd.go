...
func (c *ConnRbd) localAttachVolume() (map[string]string, error) {
	...
	devicePath, found := waitForPath(poolName, poolVolume, 1)
	if !found {
		mapArgs := []string{
			"--id", c.cephClientUser,
			"-m", c.monitorAddr,
		...
		res, err := utilsExecute("rbd", mapArgs...)
		if err != nil {
			klog.Error("Encounter error when map cinder rbd volume to host node: ", err, res)
			return nil, err
		}
		devicePath, found = waitForPath(poolName, poolVolume, 5)
		if !found {
			return nil, fmt.Errorf("Encounter error when map cinder rbd volume to host node, related host device not fount.")
		}
		klog.Infof("Success map cinder rbd volume %s/%s to host node %s.", poolName, poolVolume, devicePath)
	}
	return map[string]string{"path": devicePath, "type": "block"}, nil
